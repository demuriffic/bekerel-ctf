...
import { RefreshCw, Zap, Flame, Clock } from 'lucide-react';
...
export default function FeedList({ initialFeed }: FeedListProps) {
  ...
  useEffect(() => {
    const interval = setInterval(() => fetchFeed(false), 15000);
          ...
          feed.map((item) => (
            ...
            >
              ...
              <div className="text-xs font-mono-code text-gray-500 self-end sm:self-center flex items-center gap-1.5">
                ...
                <span>
                  {new Date(item.solvedAt).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit',
                    second: '2-digit',
                  })}
